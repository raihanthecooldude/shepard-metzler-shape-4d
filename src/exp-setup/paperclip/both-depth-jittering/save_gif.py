...
def save_right_gif(trial_data):
    """Save the rotating right shape as a looping GIF."""
    right_frames = trial_data["right_frames"]
    right_depths = trial_data["right_depths"]
    trial_idx = trial_data["trial"]
    n_frames = len(right_frames)

    # stable axis limits and depth range across all frames (no jump/pulse)
    all_pts = np.concatenate(right_frames, axis=0)
    margin = 0.5
    x_min, x_max = all_pts[:, 0].min() - margin, all_pts[:, 0].max() + margin
    y_min, y_max = all_pts[:, 1].min() - margin, all_pts[:, 1].max() + margin

    all_depths = np.concatenate(right_depths)
    d_min, d_max = all_depths.min(), all_depths.max()

    fig_g, ax_g = plt.subplots(figsize=(5, 6))
    line, = ax_g.plot([], [], '-', color='red', linewidth=2)
    scatter = ax_g.scatter([], [], color='black', zorder=3)
    ...
    # ping-pong through the frames so the rotation loops back and forth smoothly
    cycle = 2 * (n_frames - 1)

    def update(i):
        idx = i if i < n_frames else cycle - i
        pts = right_frames[idx]
        line.set_data(pts[:, 0], pts[:, 1])
        scatter.set_offsets(pts)
        scatter.set_sizes(depth_to_sizes(right_depths[idx], d_min, d_max))
        return [line, scatter]

    anim = FuncAnimation(fig_g, update, frames=cycle,
                         interval=ROTATE_INTERVAL_MS, blit=True)

    out_gif = os.path.join(
        SAVE_DIR_IMG,
        f"{trial_idx}_{trial_data['path']}_{trial_data['deg2']}.gif",
    )
    fps = 1000.0 / ROTATE_INTERVAL_MS
